...
namespace InfluxDB3.Client
{
    public interface IInfluxDBClient : IDisposable
    {
        ...
        /// specified in <c>ClientConfiguration</c>, including any fields with changed default values.  <c>WritePrecision</c> values
        /// may still be taken from <c>ClientConfiguration</c> if not defined in this argument.
        /// </param>
    ...
    public class InfluxDBClient : IInfluxDBClient
    {
            ...
            CancellationToken cancellationToken = default, WriteOptions? writeOptions = null)
        {
            ...
            if (writeOptions != null)  // implies writeOptionsFinal comes from writeOptions _because_ NOT NULL
            {
                if (gzipHandler.GetThreshold() != writeOptionsFinal?.GzipThreshold)
                {
